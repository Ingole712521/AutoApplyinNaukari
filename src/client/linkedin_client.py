...
class LinkedInApplyClient:
    ...
    def start(self) -> None:
        ...
        if not self.cookies_file.exists():
            raise FileNotFoundError(f'Missing {self.cookies_file}. Run: python linkedin_login.py')
        cookies = json.loads(self.cookies_file.read_text(encoding='utf-8'))
        for cookie in cookies:
            try:
                self.driver.add_cookie({'name': cookie['name'], 'value': cookie['value'], 'domain': cookie.get('domain', '.linkedin.com')})
            except Exception:
                pass
        self.driver.refresh()
        time.sleep(3)
    ...
    def search_jobs(self, keyword: str, location: str, max_jobs: int) -> list[LinkedInJob]:
        ...
        for card in cards:
            if len(jobs) >= max_jobs:
                break
            try:
                link_el = card.find_element(By.CSS_SELECTOR, "a[href*='/jobs/view/'], a.job-card-container__link, a.base-card__full-link")
                href = link_el.get_attribute('href') or ''
                if not href or href in seen_urls:
                    continue
                seen_urls.add(href)
                job_id_match = re.search('/jobs/view/(\\d+)', href)
                ...
                company = 'N/A'
                try:
                    company_el = card.find_element(By.CSS_SELECTOR, '.job-card-container__company-name, .base-search-card__subtitle, h4.base-search-card__subtitle')
                ...
                try:
                    ...
                    pass
                jobs.append(LinkedInJob(job_id=job_id, title=title or 'N/A', company=company, location=location_text, job_url=href.split('?')[0], apply_link=href.split('?')[0]))
            except NoSuchElementException:
                continue
        return jobs
    ...
    def apply_job(self, job: LinkedInJob) -> dict:
        ...
        if not apply_btn:
            ...
            return {'status': 'failed', 'notes': f'No apply button found ({snippet})', 'external_url': ''}
        ...
        if any((x in label for x in EXTERNAL_APPLY_LABELS)):
            ...
            return {'status': 'external', 'notes': 'Redirects to company website', 'external_url': external_url or job.job_url}
        ...
        if new_handles:
            ...
            return {'status': 'external', 'notes': 'Opened company apply page', 'external_url': external_url}
        form = LinkedInEasyApplyForm(self.driver, self.profile, max_steps=LINKEDIN_EASY_APPLY_MAX_STEPS)
    ...
    def _job_page_urls(self, job: LinkedInJob) -> list[str]:
        urls: list[str] = []
        match = re.search('(\\d+)', job.job_id or '')
        if match:
            jid = match.group(1)
            urls.append(f'https://www.linkedin.com/jobs/search/?currentJobId={jid}&f_AL=true')
    ...
    def _prepare_job_page(self) -> None:
        ...
        try:
            wait.until(EC.presence_of_element_located((By.CSS_SELECTOR, '.jobs-unified-top-card, .job-view-layout, h1.t-24, h2.job-details-jobs-unified-top-card__job-title')))
        except TimeoutException:
            pass
        for _ in range(3):
            self.driver.execute_script('window.scrollTo(0, 0);')
            time.sleep(0.4)
        for selector in ('.jobs-unified-top-card', '.jobs-apply-button--top-card', '.jobs-s-apply', '.jobs-details__main-content'):
            try:
    ...
    def _button_label(self, element) -> str:
        parts = [element.text, element.get_attribute('aria-label'), element.get_attribute('innerText'), element.get_attribute('title')]
        try:
            parts.append(self.driver.execute_script("return (arguments[0].innerText || arguments[0].textContent || '');", element))
        except Exception:
            pass
    ...
    def _collect_apply_button_candidates(self) -> list:
        ...
        def add(el) -> None:
            try:
                ...
                pass
        css_selectors = ['button.jobs-apply-button', 'button.jobs-apply-button--top-card', 'button.jobs-s-apply__application-button', 'div.jobs-apply-button--top-card button', 'div.jobs-s-apply button', "button[aria-label*='Easy Apply']", "button[aria-label*='easy apply']", "button[aria-label*='Apply']", 'a.jobs-apply-button']
        xpath_selectors = ["//button[contains(@class, 'jobs-apply-button')]", "//button[contains(., 'Easy Apply')]", "//button[.//span[contains(normalize-space(), 'Easy Apply')]]", "//span[contains(normalize-space(), 'Easy Apply')]/ancestor::button[1]", "//div[contains(@class, 'jobs-apply-button')]//button", "//div[@role='button' and contains(., 'Easy Apply')]"]
