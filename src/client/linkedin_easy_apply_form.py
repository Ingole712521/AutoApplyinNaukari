...
MODAL_SELECTORS = ('div.jobs-easy-apply-modal', "div[data-test-modal-id='easy-apply-modal']", 'div.artdeco-modal.jobs-easy-apply-modal')
...
class LinkedInEasyApplyForm:
    ...
    def wait_for_modal(self, timeout: int=15) -> bool:
        try:
            WebDriverWait(self.driver, timeout).until(lambda d: self.get_modal() is not None)
            return True
        except Exception:
            return False
    ...
    def _submitted_success(self) -> bool:
        ...
        return any((x in src for x in ('application sent', 'your application was sent', 'application submitted', 'you applied')))
    ...
    def _fill_linkedin_dropdowns(self, modal: WebElement, force: bool) -> None:
        toggles = modal.find_elements(By.CSS_SELECTOR, "div[data-test-text-select-toggle], button[aria-haspopup='listbox']")
        for toggle in toggles:
            try:
                ...
                time.sleep(0.4)
                options_els = self.driver.find_elements(By.CSS_SELECTOR, "div[role='option'], li[role='option']")
    ...
    def _element_question_text(self, element: WebElement) -> str:
        ...
        if eid:
            try:
                ...
                pass
        parts.extend([element.get_attribute('aria-label') or '', element.get_attribute('placeholder') or ''])
        try:
            group = element.find_element(By.XPATH, "./ancestor::div[contains(@class, 'fb-dash-form-element')][1]")
            parts.append(group.text)
        except NoSuchElementException:
            pass
        text = ' '.join((p for p in parts if p)).strip()
        return re.sub('\\s+', ' ', text)[:500]
    ...
    def _button_label(self, element: WebElement) -> str:
        parts = [element.text, element.get_attribute('aria-label'), element.get_attribute('innerText')]
