...
from src.utils.cookie_auth import apply_cookies_to_session, cookies_to_dict, get_nauk_at_token, load_cookies
import requests
...
DEFAULT_HEADERS = {'accept': 'application/json', 'appid': '105', 'clientid': 'd3skt0p', 'content-type': 'application/json', 'referer': 'https://www.naukri.com/nlogin/login', 'systemid': 'jobseeker', 'x-requested-with': 'XMLHttpRequest'}
UPLOAD_HEADERS = {'accept': 'application/json, text/javascript, */*; q=0.01', 'appid': '105', 'origin': 'https://www.naukri.com', 'referer': 'https://www.naukri.com/', 'systemid': 'fileupload'}
OTP_HEADERS = {'accept': 'application/json', 'appid': '100', 'content-type': 'application/json', 'referer': 'https://www.naukri.com/nlogin/login?URL=//www.naukri.com/mnjuser/recommendedjobs', 'sec-ch-ua': '"Chromium";v="146", "Not-A.Brand";v="24", "Google Chrome";v="146"', 'sec-ch-ua-mobile': '?0', 'sec-ch-ua-platform': '"Windows"', 'systemid': 'jobseeker', 'user-agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/146.0.0.0 Safari/537.36', 'x-requested-with': 'XMLHttpRequest'}
...
class NaukriLoginClient:
    ...
    def _login_request(self):
        return self.session.post(LOGIN_URL, headers=self._build_headers(), json={'username': self.username, 'password': self.password})
    ...
    def _validate_session(self) -> bool:
        try:
            res = self.session.get(DASHBOARD_URL, headers=self._build_headers(auth=True))
    ...
    def _verify_otp_request(self, username: str, otp: str, is_mobile: bool):
        payload = {'username': username, 'token': otp, 'flowId': 'login', 'isLoginByEmail': not is_mobile, 'isLoginByMobile': is_mobile}
        return self.session.post(OTP_VERIFY_URL, headers=OTP_HEADERS, json=payload)
    ...
    def _send_otp_request(self, username: str, is_mobile: bool):
        payload = {'username': username, 'flowId': 'login', 'isLoginByEmail': not is_mobile, 'isLoginByMobile': is_mobile}
        otp_header = self._build_headers()
        otp_header['appid'] = '100'
        return self.session.post(OTP_SEND_URL, headers=otp_header, json=payload)
    ...
    def _validate_file_request(self, filename, file_bytes, form_key, file_key):
        return requests.post(FILE_VALIDATION_URL, headers=UPLOAD_HEADERS, files={'file': (filename, BytesIO(file_bytes), 'application/pdf')}, data={'formKey': form_key, 'fileName': filename, 'uploadCallback': 'true', 'fileKey': file_key})
    ...
    def update_resume(self, resume_file):
        ...
        headers = self._build_headers(auth=True, extra={'accept-encoding': 'gzip, deflate, br, zstd', 'accept-language': 'en-US,en;q=0.9', 'content-type': 'application/json', 'origin': 'https://www.naukri.com', 'referer': 'https://www.naukri.com/mnjuser/profile', 'systemid': '105', 'x-http-method-override': 'PUT'})
    ...
    def update_profile(self, headline: str=None, name: str=None, summary: str=None):
        pid = self.fetch_profile_id()
        headers = self._build_headers(auth=True, extra={'accept-encoding': 'gzip, deflate, br, zstd', 'accept-language': 'en-US,en;q=0.9', 'origin': 'https://www.naukri.com', 'referer': 'https://www.naukri.com/mnjuser/profile?id=&altresid', 'systemid': '105', 'x-http-method-override': 'PUT', 'x-requested-with': 'XMLHttpRequest'})
        profile_fields = {}
    ...
    def _fetch_history_request(self, page_size, days, page_number, mobile=False):
        headers = {'accept': 'application/json', 'appid': '135' if mobile else '107', 'systemid': '135' if mobile else '107', 'content-type': 'application/json', 'x-requested-with': 'XMLHttpRequest', 'referer': 'https://www.naukri.com/apply/historypage' if mobile else 'https://www.naukri.com/myapply/historypage', 'authorization': f'Bearer {self.naukri_session.bearer_token}'}
        if mobile:
            headers['clientid'] = 'm0b5'
        params = {'pageSize': page_size, 'days': days, 'pageNumber': page_number}
        if not mobile:
            params['filterInfo'] = 2
    ...
    def parse_history(self, raw: dict) -> list[ApplicationHistory]:
        ...
        for item in raw.get('applyDetails', []):
            statuses = [ApplicationStatus(status_id=s['statusId'], status_value=s['statusValue'], date_time=s['dateTime']) for s in item.get('status', [])]
            rating = item.get('companyRating', {})
            results.append(ApplicationHistory(job_id=item['jobId'], job_title=item['jobTitle'], company=item['company'], location=item['location'], apply_type=item['applyType'], is_open=item['isOpen'] == 'true', ars_score=item.get('arsScore', 0), star_rating=item.get('starRating', '0'), job_type=item.get('jobType', ''), statuses=statuses, company_rating=float(rating['AggregateRating']) if rating else None, logo_path=item.get('logoPath')))
        return results
