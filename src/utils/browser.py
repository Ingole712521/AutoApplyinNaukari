...
def find_brave_binary() -> str | None:
    ...
    if env_path and Path(env_path).is_file():
        return env_path
    candidates = [Path('C:\\Program Files\\BraveSoftware\\Brave-Browser\\Application\\brave.exe'), Path('C:\\Program Files (x86)\\BraveSoftware\\Brave-Browser\\Application\\brave.exe'), Path.home() / 'AppData/Local/BraveSoftware/Brave-Browser/Application/brave.exe']
...
def create_webdriver(*, headless: bool=False) -> webdriver.Chrome:
    ...
    if USE_BRAVE_BROWSER:
        ...
        if brave:
            ...
            print('Warning: Brave not found. Install Brave or set BRAVE_BINARY_PATH in config.py. Falling back to Chrome.')
