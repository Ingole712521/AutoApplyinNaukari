...
def sync_excel_download(local_path: Path) -> bool:
    ...
    if not token:
        return False
    try:
        res = requests.get(f'https://blob.vercel-storage.com/{BLOB_EXCEL_PATH}', headers={'Authorization': f'Bearer {token}', 'x-api-version': '7'}, timeout=60)
...
def sync_excel_upload(local_path: Path) -> str | None:
    ...
    try:
        ...
        res = requests.put(f'https://blob.vercel-storage.com/{BLOB_EXCEL_PATH}', headers={'Authorization': f'Bearer {token}', 'x-api-version': '7', 'Content-Type': 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'}, data=body, timeout=120)
        res.raise_for_status()
        data = res.json()
        url = data.get('url')
