...
class ExcelJobLogger:
    ...
    def append_job(self, job, keyword: str, status: str, notes: str='', applied_at: str='', platform: str='Naukri', external_apply_url: str='') -> None:
        ...
        ws.append([excel_cell_value(platform), excel_cell_value(self.run_date), excel_cell_value(self.run_time), excel_cell_value(job.job_id), excel_cell_value(job.title), excel_cell_value(job.company), excel_cell_value(job.location), excel_cell_value(getattr(job, 'experience', '') or 'N/A'), excel_cell_value(getattr(job, 'salary', '') or 'N/A'), excel_cell_value(getattr(job, 'posted_date', '') or 'N/A'), excel_cell_value(keyword), excel_cell_value(skills), excel_cell_value(apply_link), excel_cell_value(external_apply_url), excel_cell_value(status), excel_cell_value(applied_at), excel_cell_value(notes)])
    ...
    def append_run_summary(self, stats: dict, total_found: int, platform: str='') -> None:
        ...
        ws.append([platform or '', self.run_date, self.run_time, '', label, '', '', '', '', '', '', '', '', '', 'Summary', '', f"Found={total_found}; Applied={stats.get('applied', 0)}; Already Applied={stats.get('skipped_applied', 0)}; Company Skipped={stats.get('skipped_company', 0)}; External={stats.get('skipped_external', 0)}; Failed={stats.get('failed', 0)}"])
        wb.save(self.filepath)
