...
from config import APPLY_DELAY_SEC, ENABLE_LINKEDIN, ENABLE_NAUKRI, EXCEL_FILE, EXPERIENCE_YEARS, JOB_AGE_DAYS, LINKEDIN_COOKIES_FILE, LINKEDIN_HEADLESS, LINKEDIN_LOCATION, LINKEDIN_MAX_JOBS_PER_QUERY, LINKEDIN_SEARCH_QUERIES, LOOP_INTERVAL_MINUTES, PAGES_PER_QUERY, SEARCH_DELAY_SEC, SEARCH_QUERIES, SKIP_IF_COMPANY_ALREADY_APPLIED, TITLE_KEYWORDS
...
def fetch_naukri_jobs(jc: NaukriJobClient) -> list[tuple]:
    seen: set[str] = set()
    results: list[tuple] = []
    print_section(f'Naukri search — {len(SEARCH_QUERIES)} queries, {PAGES_PER_QUERY} page(s) each, exp={EXPERIENCE_YEARS}yr')
    for query in SEARCH_QUERIES:
        keyword = query['keyword']
        location = query.get('location', '')
        for page in range(1, PAGES_PER_QUERY + 1):
            try:
                jobs = jc.search_jobs(keyword=keyword, location=location, experience=EXPERIENCE_YEARS, job_age=JOB_AGE_DAYS, page=page)
...
def main() -> int:
    ...
    mode = 'search only' if args.search_only else 'once' if args.once else f'every {interval} minutes'
